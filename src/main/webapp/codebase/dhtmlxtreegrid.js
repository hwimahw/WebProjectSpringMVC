...
function dhtmlxHierarchy(){
		...
		this.addAfter=function(id,parentId,afterId,fix){  
			...
			if (afterId)
				var ind=this.get[afterId].mainPage+(fix?0:1);
			...
			if (afterId){
				for (var i=ind; i<z.childs.length; i++) z.childs[i].mainPage++;
		...
		this.remove=function(id){  
			...
			z.childs=[];	
			z.parent.childs=z.parent.childs.slice(0,z.mainPage).concat(z.parent.childs.slice(z.mainPage+1));
			for (var i=z.mainPage; i<z.parent.childs.length; i++)
				z.parent.childs[i].mainPage--;
			delete this.get[id];
		}
...
if (!this._realfake)	
	this._h2.forEachChild(0,function(x){
		if (x.buff.tagName=="TR"){
			var cs=(this._cssSP?(x.level%2):(x.mainPage%2))?this._cssUnEven:this._cssEven;
...
dhtmlXGridObject.prototype.moveRowUDTG = function(id,dir){ 
	...
	var p=x.parent.childs[x.mainPage+dir]
